...
def main():
    ...
    parser.add_argument('--genome', required= True, help='Genome FASTA file (.fna or .fna.gz)')
    parser.add_argument('--counts_global', required= True, help='Either a kmer counts file or a scrubbed kmers file if map_scrubbed_kmers')
    parser.add_argument('--counts_individual', required= True, help='Either a kmer counts file or a scrubbed kmers file if map_scrubbed_kmers')
    ...
    parser.add_argument('--percentage', type=float, default=0.01, help='Percentile threshold for rare kmer selection (default: 0.05)')
    ...
    if args.map_scrubbed_kmers_only:
        ...
        print(f"Locating unique kmers on genome ({len(df_inform_singletons)})")
        singleton_kmers = set(df_inform_singletons["#kmer"].to_list())

        df_locations_singletons ,_ = build_mapped_kmers_ahocorasick(records, singleton_kmers, terminal_dist=args.terminal_dist)
        print('dropping terminal kmers')
        df_locations_singletons = df_locations_singletons.loc[df_locations_singletons['terminal_kmer'] == False] 
        ...
        print(f'Selected {len(selected)} kmers from unique at kmer overlap {args.kmer_overlap}')
        print('creating pairs of selected kmers')
        create_all_pairs(selected,
                         output_dir = args.output_dir, 
                         basename = f"{args.basename}.inform_kmer_pairs.singletons.parquet",
                         batch_size=1_000_000,
                         max_kmers=args.max_kmers)

        if len(selected) < args.max_kmers:
            # read the median list length of the counts_individual
            mean = (
                        pl.scan_csv(args.counts_individual, separator='\t')
                        .with_columns(pl.col('list_scrub_id').str.split(',').cast(pl.List(pl.UInt32))
                            .list.len().alias('n_presence'))
                        .select(pl.col('n_presence').mean())
                        .collect(engine='streaming')
                        .item()
                    )
            print(mean)
            presence_t = max(2, math.ceil(mean))
            print(presence_t)
            ...
            # map kmers
            # make independent 
            # add to all kmers, if not enough continue with higher presence
