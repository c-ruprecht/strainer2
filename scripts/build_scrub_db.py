...
def main():
    ...
    parser.add_argument('--min_jaccard', default = 0.8, help = 'min jaccard distance to use for full kmer comparison')
    ...
    # Create target sample database:
    df_targetsamples = pd.read_csv(args.target_samples)
    assembly_path = os.path.join(args.scrub_db_path, 'target_samples')
    os.makedirs(assembly_path, exist_ok=True)
    li_accessions = df_targetsamples['match_name'].str.split(' ').str[0].unique()
    print(f'Found {len(li_accessions)} unique accession numbers to download for target metagenome samples')
    download_by_accession(li_accessions, assembly_path)

    # Build drug scrub databases
    ...
    # Do fast precomparison with sourmash
    ...
    if not df_kmer.empty:
        ...
        ### Remove all assemblies with high similarity 
        print('Removing assemblies from database in droplist')
        n_deleted = 0
        for name in li_droplist:
            fpath = os.path.join(assembly_path, name)
            if os.path.exists(fpath):
                os.remove(fpath)
                n_deleted += 1
            else:
                print(f"[build] Not found, skipping: {name}", file=sys.stderr)
        print(f"[build] Deleted {n_deleted}/{len(li_droplist)} assemblies.", file=sys.stderr)
